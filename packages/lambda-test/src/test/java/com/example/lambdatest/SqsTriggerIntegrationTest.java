  ...
  void testSqsEventSourceMapping() throws Exception {
    ...
    try (LambdaClient lambda =
      ...
      String sourceQueueArn =
          String.format("arn:aws:sqs:%s:000000000000:source-queue", localstack.getRegion());
      lambda.createEventSourceMapping(
          em -> em.functionName("SqsHandlerTest").eventSourceArn(sourceQueueArn).batchSize(1));
      // ACTIVEになるまで待機
      await()
          ...
          .until(
              () ->
                  lambda.getFunction(r -> r.functionName("SqsHandlerTest")).configuration().state()
                      == State.ACTIVE);
    }
    // メッセージ送信でトリガーを発動
    sqs.sendMessage(
        sm -> sm.queueUrl(sourceQueueUrl).messageBody("{\"name\":\"Alice\",\"age\":30}"));
    // フォワード結果を待機・検証
    await()
